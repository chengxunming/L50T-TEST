...
void CAN_Filter_Config(void)
{
   CAN_FilterInitTypeDef  CAN_FilterInitStructure;
    ...
    CAN_FilterInitStructure.CAN_FilterIdHigh= ((uint32_t)(0x07<<21)&0xffff0000)>>16;				//要过滤的ID高位 
    CAN_FilterInitStructure.CAN_FilterIdLow= (CAN_ID_STD|CAN_RTR_DATA)&0xffff; //要过滤的ID低位 
    CAN_FilterInitStructure.CAN_FilterMaskIdHigh=0xEF1F;//0x7FFF;			//过滤器高16位除心跳帧之外必须匹配
...
//发送CAN数据帧--SSU发送--单帧
uint8_t Send_CAN_DataFrame(uint8_t addr,uint8_t *ptr,uint8_t len) //ptr: 数据指针. len: 数据长度 
{
	uint16_t i=0;
