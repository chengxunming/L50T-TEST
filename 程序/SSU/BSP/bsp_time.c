...
static uint8_t Convert_num=0,AverageCnt=0;
//定时器初始化
void Time_Init(void)
{
...
{
	if(state_ADC==ADC_STA_COL)
	{
		Convert_num++;    //采集次数+1
		Get_Adc(Convert_num-1);
		if((Convert_num%50)==0)   //50ms计时到
		{
			Convert_num=0;
			AverageCnt++;
			Get_VoltRMS(AverageCnt-1);
			if(AverageCnt==20)//每秒上传数据
			{
				AverageCnt = 0;
				Cal_Average();
				state_ADC=ADC_STA_Ok;//AD采集完成
			} 
		}
