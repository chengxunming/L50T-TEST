...
void CAN_Filter_Config(void)
{
   CAN_FilterInitTypeDef  CAN_FilterInitStructure;
    ...
    CAN_FilterInitStructure.CAN_FilterIdHigh= ((uint32_t)(SSU_CAN_ID<<21)&0xffff0000)>>16;				//要过滤的ID高位 
...
//发送CAN数据帧--PDU发送--单帧
uint8_t Send_CAN_DataFrame_SSU(uint8_t *ptr,uint8_t len) //ptr: 数据指针. len: 数据长度 
{
	uint16_t i=0;
...
void Receive_CAN_DataFrame(CanRxMsg* canRx,uint8_t num)
{
	...
	{
		case FUN_CAN:
			SSU_FunCan_Respone_Deal(&receive_Frame);
			break;
		case FUN_RESULT:
