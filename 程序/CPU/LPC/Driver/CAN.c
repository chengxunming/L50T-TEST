...
//	for(uint8_t i=0;i<CAN_RxMsg[0].len;i++)
//	{
//		canUartData[candataLen++]=CAN_RxMsg[0].data[i];
//	}
  ...
  if (icr & (1 << 0)) {                          /* CAN Controller #2 meassage is received */
    ...
    LPC_CAN2->CMR = (1 << 2);                    /* Release receive buffer */
	 ...
	 for(uint8_t i=0;i<CAN_RxMsg[1].len;i++)
	{
		canUartData[candataLen++]=CAN_RxMsg[1].data[i];
	}
